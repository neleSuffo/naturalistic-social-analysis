...
def classify_frames(row, results_df, included_rules=None, mode="tertiary"):
    """
    ...
    is_audiobook = bool(row[AUDIOBOOK_FLAG]) 
    ...
    # --- TIER 1: INTERACTING (The "Active" Rules) ---
    active_rules = []
...
def main(db_path: Path, output_dir: Path, hyperparameter_tuning: False, included_rules: list = None, mode: str = "tertiary") -> dict:
    ...
    print(f"🔄 Running {mode} multimodal social interaction analysis...")
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        results = all_data.apply(lambda row: classify_frames(row, all_data, included_rules, mode=mode), axis=1)
        ...
        final_classification_results = all_data.apply(
            lambda row: classify_frames(row, all_data, included_rules, mode=mode), axis=1
        )
