...
def evaluate_model(model, dataloader, criterion):
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            for i, label_name in enumerate(cfg.LABELS):
                ...
                # Debug: Print some prediction statistics every 100 batches
                if batch_idx % 100 == 0:
                    ...
                    print(f"    {label_name}: pos_preds={pos_preds}/{len(preds)}, pos_targets={pos_targets}/{len(targets)}, avg_prob={avg_prob:.3f}, max_prob={max_prob:.3f}, min_prob={min_prob:.3f}")
    ...
    # Debug: Print prediction statistics before calculating metrics
