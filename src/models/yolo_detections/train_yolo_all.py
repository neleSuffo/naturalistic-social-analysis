...
experiment_name = timestamp + "_yolo_all_finetune_with_augment_and_earlystop"
output_dir = YoloPaths.all_output_dir / experiment_name

# Train the model with a cosine annealing learning rate scheduler
model.train(
    data=str(YoloPaths.all_data_config_path),
    epochs=200,  # Total number of epochs
    imgsz=640,  # Image size
    batch=16,   # Batch size
    project=str(YoloPaths.all_output_dir),  # Output directory
...
script_copy = output_dir / "train_yolo_all.py"
