...
In this chapter we present the results of two model architectures applied to the ChildLens dataset. While the dataset can be used for multimodal activity analysis, we focus on two specific tasks: temporal activity localization using video data and voice type classification using audio data. We apply the Boundary-Matching Network (BMN) for temporal activity localization and the Voice Type Classifier (VTC) for voice type classification. Both models were trained from scratch on the ChildLens dataset to provide initial results and a benchmark for future research. A short architecture overview of the models is presented in figure \ref{fig:architecture-overview} and figure \ref{fig:bmn-architecture}.
