...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{file\_path }\OtherTok{\textless{}{-}} \StringTok{"quantex\_share\_data\_sheet.xlsx"}
\NormalTok{data }\OtherTok{\textless{}{-}} \FunctionTok{read\_excel}\NormalTok{(file\_path)}

\CommentTok{\# Remove duplicates}
\NormalTok{unique\_data }\OtherTok{\textless{}{-}}\NormalTok{ data }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{distinct}\NormalTok{(ID, Minutes\_per\_ID, }\AttributeTok{.keep\_all =} \ConstantTok{TRUE}\NormalTok{)}

\NormalTok{unique\_data}\SpecialCharTok{$}\NormalTok{ID }\OtherTok{\textless{}{-}} \FunctionTok{as.factor}\NormalTok{(unique\_data}\SpecialCharTok{$}\NormalTok{ID)}

\FunctionTok{ggplot}\NormalTok{(unique\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ ID, }\AttributeTok{y =}\NormalTok{ Minutes\_per\_ID, }\AttributeTok{fill =}\NormalTok{ ID)) }\SpecialCharTok{+} 
  \FunctionTok{geom\_bar}\NormalTok{(}\AttributeTok{stat =} \StringTok{"identity"}\NormalTok{) }\SpecialCharTok{+} 
  \FunctionTok{theme\_minimal}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_fill\_hue}\NormalTok{(}\AttributeTok{c =} \DecValTok{30}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Minutes of video recordings per child"}\NormalTok{,}
       \AttributeTok{x =} \StringTok{"Child ID"}\NormalTok{,}
       \AttributeTok{y =} \StringTok{"Minutes per child"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme}\NormalTok{(}\AttributeTok{axis.text.x =} \FunctionTok{element\_blank}\NormalTok{(),}
        \AttributeTok{legend.position =} \StringTok{"none"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}

\includegraphics{ChildLens_paper_files/figure-latex/import-and-plot-1.pdf}
...
The videos were manually annotated by native German speakers who watched each video and labeled the activities present in the footage. The annotators marked the start and end points of each activity, ensuring that the annotations were accurate and detailed. The labeling process was conducted using the SuperAnnotate platform, which allowed for efficient annotation and review of the videos. To ensure the quality of the annotations, \textbf{\ldots. review system?}
In a setup meeting, the annotators received instructions and our labeling strategy to follow. Before starting the annotation process, the annotators labeled 25 videos to get a feeling for the task. The annotations were then reviewed by us and feedback was shared. In total, three feedback loops were conducted to ensure that the annotators followed the labeling strategy.
