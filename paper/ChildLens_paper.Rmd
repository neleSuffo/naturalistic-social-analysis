...
my.data <- "Dataset           | Recall  | Recall  | Recall  | AR@100  | AUC
...
# Merge the three 'Recall' columns into one (averaging them)
df$Recall <- rowMeans(df[, c("Recall", "Recall.1", "Recall.2")], na.rm = TRUE)

# Remove the original three 'Recall' columns
df <- df[, !names(df) %in% c("Recall", "Recall.1", "Recall.2")]

# Update the column names if necessary
names(df)[names(df) == "Recall"] <- "Recall (Merged)"
