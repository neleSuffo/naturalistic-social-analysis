...
```{r statistics-plot, echo=FALSE, dpi=600, fig.align='center', fig.cap="\\textbf{A} – Vest with the embedded camera worn by the children, \\textbf{B} – SuperAnnotate platform utilized for video annotation, \\textbf{C} – Activity classes in the ChildLens dataset."}
audio_table <- data.frame(
  `Audio-based Activity Class` = c("Child talking", "Other person talking", "Overheard Speech", "Singing/Humming", "Listening to music/audiobook"),
  `Instance Count` = c(100, 100, 100, 100, 100),
  `Total Duration (min)` = c(100, 100, 100, 100, 100),
  stringsAsFactors = FALSE
) %>%
...
# Create table for video-based activity classes
video_table <- data.frame(
  ...
  `Total Duration (min)` = c(100, 100, 100, 100),
  stringsAsFactors = FALSE
) %>%
  kable("html", escape = FALSE) %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
...
# Create ggplot for Minutes per Child
minutes_plot <- ggplot(unique_data, aes(x = Minutes_per_ID)) +
...
# Use patchwork to layout the components
top_row <- (wrap_elements(audio_table) + wrap_elements(video_table)) + 
  plot_layout(widths = c(0.5, 0.5)) # Adjust widths of the two tables

final_layout <- (top_row / minutes_plot) +
  plot_layout(heights = c(1, 1))  # Adjust row heights
...
```{r minutes-per-child, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Video recording duration (in minutes) per child ID.", fig.height=3, fig.width=4}
unique_data$ID <- as.factor(unique_data$ID)
...
# Save the plot
ggsave(filename = file_path, plot = plot, width = 4, height = 3, dpi = 300)
```
...
```{r activity-classes-statistics, echo=FALSE, message=FALSE, warnings=FALSE, results='asis', eval=TRUE}
df <- data.frame(
  `Activity Class` = c("Child talking", "Other person talking", "Overheard Speech", "Singing/Humming", "Listening to music/audiobook", "Playing with object", "Reading book", "Making music", "Pretend Play", "Playing without object", "Watching something", "Drawing", "Crafting things", "Dancing"),
  `Instance Count` = c(100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100),
  `Total Duration` = c(100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100),
  stringsAsFactors = FALSE
)
colnames(df) <- c("Activity Class", "Instance Count", "Total Duration (min)")

df %>%
  kable("pipe", caption = "Statistics of ChildLens Activity Classes") %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
  row_spec(0, bold = TRUE)
```
...
```{r quantex-minutes-per-child, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE, fig.cap="Video recording duration (in minutes) per child ID."}
