...
#### Activity Classes
The ChildLens dataset contains a total of 14 activity and 5 location classes. The activities are based on the actions of the child in the video and can be divided into _person-only_ activities, such as "child talking" or "other person talking, and _person-object_ activities, such as "drawing" or "playing with object". You can find a brief description of each class in the appendix. The activities can be further divided into _audio-based_, _visual-based_, and _multimodal_ activities, as presented in figure \@ref(fig:activity-classes). The following list provides an overview of the different activity types:
...
subject_infos <- read_csv2("subjects_quantex_share.csv")
...
- Child is invited to draw/craft something [MB: hatten wir nicht noch dass die Kinder verschiedene räuzme besuchen sollen?]

The videos were recorded over a period of `r time_span_months` months, resulting in a total of `r sum_hours` hours of video footage [MB:die info hast du oben schon].

[MB: figure camera-worn noch hervorheben wo die Kamera war]
...
3. **Review process**: After the initial annotations are completed, the annotations are reviewed by the internal team [MB:meinst du uns? dann besser research team, oder gleich first author of the paper] to ensure that they are accurate and complete. Any discrepancies or errors are corrected before the final submission.
...
[MB:figure bmn-architecture - hast du die selbst gemacht oder aus einem anderen paper? falls ja, müssten wir uns erst die erlaubnis dafür holen. ich denke aber es braucht nicht unbedingt eine figure für die beiden model architectures weil das ja nicht der fokus ist]
...
### Implementation Details
The Voice Type Classifier was applied to the ChildLens dataset. A total of `r num_audio_files` raw files, with a total duration of `r sum_hours`, were processed by the model in `r vtc_minutes_run` minutes [MB: den teil braucht es nicht, ist ja egal wie schnell das läuft :) - der ganze absatz kann raus. Wenn dann würde ich unter implementation details nur sahcen beschreiben die anderen helfen deine ergebnisse zu reproduzieren]. 
