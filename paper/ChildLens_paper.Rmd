...
#### Statistics
The ChildLens dataset comprises **xx** hours of video material recorded by **xx** children aged 3 to 5 years. For each child, the duration of recorded video material varies between **x** and **x** hours. A detailed distribution of the video material can be found in figure \@ref(fig:video-distribution). 
...
```{r import-and-plot, echo=TRUE, message=FALSE, warning=FALSE}
file_path <- "quantex_share_data_sheet.xlsx"
...
ggplot(unique_data, aes(x = ID, y = Minutes_per_ID, fill = ID)) + 
  geom_bar(stat = "identity") + 
  theme_minimal() +
  scale_fill_hue(c = 30) +
  labs(title = "Minutes of video recordings per child",
       x = "Child ID",
       y = "Minutes per child") +
