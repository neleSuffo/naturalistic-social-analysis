...
# Introduction
In developmental psychology, only a small body of research provides direct insights into children’s day-to-day experiences, despite their recognized significance in shaping development [@rogoffImportanceUnderstandingChildrens2018]. 



Conventional methods, like brief laboratory sessions,offer only a superficial assessment of the multi-layered construct of everyday experiences. Recent technological advances, notably wearable recording devices, provide an opportunity to capture these experiences on a larger scale through long-term
recordings in the children’s natural environments at home or in kindergarten. However, handling the huge amount of data is a challenge that we want to overcome by implementing an automated analysis procedure using Machine Learning and Computer Vision algorithms. Many studies show promising results in extracting information about detected objects/people in video data and identified voices in audio recordings (Ansari &Singh, 2020; He et al., 2017; Lavechin et al., 2020; Redmon et al., 2015).

Recent studies exploring children’s perspectives and their perception of the surrounding environment have often focused exclusively on the audio component of the data [@sullivanSAYCamLargeLongitudinal2021; @royPredictingBirthSpoken2015], solely examined video data [@yoshidaWhatsViewToddlers2008; @saberCurriculumLearningInfant2023; @smithContributionsHeadMountedCameras2015; @borjonViewTheirOwn2018], or concentrated on infants under the age of two years [@sullivanSAYCamLargeLongitudinal2021; @tsutsuiComputationalModelEarly2020]. However, the combination of audio and video data in a multimodal dataset can provide a more comprehensive understanding of children’s activities and interactions. This multimodal approach has been successfully applied in adult activity recognition [@kapidisObjectDetectionBasedLocation2020; @truongCrossviewActionRecognition2024], but research on multimodal activity recognition in children is limited. For example, [@longBabyViewDatasetHighresolution2024] introduced the BabyView dataset, a large-scale multimodal dataset capturing egocentric experiences of children aged 6 months to 5 years. While it supports tasks like speech transcription and pose estimation, it does not address activity localization, leaving a key gap in understanding children’s daily interactions.

We therefore introduce the ChildLens dataset, a novel egocentric multimodal dataset capturing the everyday experiences of children aged 3–5 years. The dataset comprises 106 hours of video and audio material, recorded by 61 children wearing a camera-equipped vest. The dataset includes five location classes and 14 activity classes, covering audio-only, video-only, and multimodal activities. We provide detailed annotations for each activity, including the start and end times, the activity class, and whether the child is engaged alone or with somebody else. The dataset is designed to support research in developmental psychology and computer vision, offering a rich source of data for activity analysis and multimodal learning.
...
### Data Preparation
The videos were preprocessed following the MMAction2 guidelines to ensure compatibility with the model architecture. Prior to model training, we analyzed the number of instances per activity class to assess to evaluate the data sufficiency for training and testing purposes. The distribution of activity instances and their total duration across activity classes are presented in Table @ref(tab:activity-classes-statistics) in the appendix.
