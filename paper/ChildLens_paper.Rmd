...
# Introduction
In developmental psychology, children’s everyday experiences are widely recognized as crucial for shaping their cognitive, emotional, and social development [@rogoffImportanceUnderstandingChildrens2018; @carpendale2020makes; @smith2018developing] [MB: Hier kannst du gerne noch mehr citations hinzufügen, ist ja ein zentraler punkt. ich hab mal zwei hinzugefügt, aber im prinzip sagen das fast alle :) - Heyes, Tomasello, oder ein paar alte Hasen wir Vygotski oder Piaget]. For instance, @spanglerToddlersEverydayExperiences1989 shows [MB: past tense hier und überall] that toddlers’ daily interactions influence their mental and emotional dispositions and predict later mental and motivational development. Additionally, @debarbaroTenLessonsInfants2022 emphasize the dynamic and diverse nature of infants’ experiences as captured by everyday activity sensors, highlighting the need to analyze these interactions over extended periods to fully understand their patterns, variability, and developmental significance [MB: das könntest du ruhig ausführen und beschreiben wie die Daten erhoben wurden]. Despite the recognized importance of these experiences, research directly examining their developmental implications remains limited. [MB: Dieser Absatz ( und die Intro) ist ein bisschen zu kurz noch. Ich würde folgendes machen: 
* klassische Theorien (Piaget, Vygotski etc.) sagen alltagserfahrungen sind zentral - Beispiel geben 
* Neue Theorien auch (Tomasello, Heyes, etc.) - Beispiel geben 
* ABER eine direkte Verbindung zwischen Alltags erfahrung und psychologischer Entwicklung wird nur selten untersucht.
* Neuer Absatz mit Beispielen wo das gemacht wurde. hier ein paar paper als beispiel aus dem bereich Sprache und souiale Kognition, ein paar weitere wären aber gut: 
** Ruffman, T., Chen, L., Lorimer, B., Vanier, S., Edgar, K., Scarf, D., & Taumoepeau, M. (2023). Exposure to
behavioral regularities in everyday life predicts infants’ acquisition of mental state vocabulary. Developmental
Science , 26 (4), e13343. https://onlinelibrary.wiley.com/doi/pdf/10.1111/desc.13343
** Bergelson, E., Soderstrom, M., Schwarz, I. C., Rowland, C. F., Ramírez-Esparza, N., R. Hamrick, L., ... & Cristia, A.
(2023). Everyday language input and production in 1,001 children from six continents. Proceedings of the
National Academy of Sciences , 120 (52), e2300671120.
** Donnelly, S., & Kidd, E. (2021). The longitudinal relationship between conversational turn-taking and vocabulary
growth in early language development. Child Development , 92 (2), 609-625.
https://srcd.onlinelibrary.wiley.com/doi/full/10.1111/cdev.13511
...
https://www.pnas.org/doi/abs/10.1073/pnas.1419773112
** Rowe, M. L., & Goldin-Meadow, S. (2009). Differences in early gesture explain SES disparities in child vocabulary
size at school entry. Science , 323 (5916), 951-953.
** 

* Neuer Absatz: Sagen, dass ein Probelm bei diesem Ansatz die schiere Datenmenge ist. Je mehr vom Alltag man betrachten kann umso schwieriger wird es das zu machen mit tradtitionellen methoden (händisches annotieren)
* Zum Glück gibt es mittlwerweile Computermodelle die sowas automatisiert machen können. 
* Ein paar modelle besipielhaft beschreiben, und was man damit machen kann (e.g. person detection to study social interaction etc.)
* Kritisch für die Anwendung und Weiterentwicklung dieser Modelle ist aber die FREIE Verfügbarkeit von entsprechenden Datensätzen. Dann ggf. ein beispiel aus der Computer Vision Welt allgemein wo ein Datensatz dazu geführt hat das neue methoden entwickelt wurden. 
* dann weiter im Sinne des folgenden Absatzes in dem Du vorhandenene Datensätze beschreibst. Hier vor allem die die frei verfügbar sind. ROy et al zum Beispiel ist das nicht, daher sollte das nur oben erwähnt werden. 
]
...
We trained the BMN model from scratch on the ChildLens dataset to predict the start and end boundaries of the video-based activity classes. The model was implemented using MMAction2, "an open-source toolbox for video understanding based on PyTorch" [@mmaction2contributorsOpenMMLabsNextGeneration2020]. Training was conducted on a Linux server with 48 cores and 187 GB RAM. The model was optimized using the Adam optimizer with a learning rate of 0.001 and a batch size of 16. The training process involved multiple epochs, with early stopping based on validation loss to prevent overfitting.
...
[MB: irgendwo sollte noch eine section sein die beschreibt wie man Zugang zu den Videos und den Annoatationen bekommen kann]
