...
Leveraging the capabilities of the open-source pyannote library for speaker diarization [@plaquetPowersetMulticlassCross2023;@bredinPyannoteaudio21Speaker2023], we evaluated the ChildLens dataset's quality through three distinct VTC training setups:

1. **Direct Application**: Pretrained VTC model applied to ChildLens as-is.
2. **Fine-tuned:**: Pretrained VTC fine-tuned on ChildLens.
3. **Training from Scratch**: VTC trained solely on ChildLens.
...
Ranging from 0 to 1, \( F_1 \)> 0.8 score is considered good and \( F_1 \) > 0.9 as excellent.
...
apa_table(
  df,
  caption = "Cumulative Duration (in minutes) of Utterances Categorized by Voice Type",
  escape = TRUE,
)
