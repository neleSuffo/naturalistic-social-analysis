...
utterance_01_df <- read_csv("../paper/data/utterance_01_df.csv", show_col_types = FALSE) # realtive paths!
...
m_utterance_01a_df <- utterance_01_df%>% 
    mutate(child_id = factor(child_id),
           z_age = scale(age)
        )
...
m_utterance_01c_df <- utterance_01_df%>%
...
bmodel_01c <- brm(kchi_present ~ z_age * person_age_class * close_face_present_text + (1 | child_id), # remove random slopes within child just because it takes long to run them, bring them back later; also need to remove some variables becasue too little data
               data = m_utterance_01c_df,
               family = bernoulli(),
               chains = 4, 
               cores = 4,
               iter = 5000,
