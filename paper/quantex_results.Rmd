...
utterance_01b_df <- utterance_01_df%>%
    ...
    mutate(child_id = factor(child_id),
           ...
           object_class = factor(object_class, levels = c("none", "book", "toy", "kitchenware", "screen", "food", "other object", "multiple")),
           z_age = scale(age),
        )
# bayesian model - no convergence problems and more options for visuals

model_01b <- brm(kchi_present ~ z_age * object_class + ( object_class | child_id),
