...
In this research, we employed Ultralytics' YOLO11, the ``latest iteration in the Ultralytics YOLO series of real-time object detectors'' (Jocher \& Qiu, 2024). YOLO11, which was released in October 2024, has faster processing, increased efficiency, and improved spatial attention techniques to focus on critical parts of a picture compared to prior Yolo versions (Khanam \& Hussain, 2024). These improvements make YOLO11 suited for training on our egocentric dataset to detect people, which includes dynamic motions captured from camera perspectives at chest level.

\subsubsection{Dataset Splitting and Balancing}\label{dataset-splitting-and-balancing}

Data preprocessing initiated with annotation from 80 videos and a total number of 113799 frames. Before splitting the data into training, validation and testing subsets, our interest was in the proportion of frames containing annotated persons versus those without. Our analysis showed that 45.25\% of the frames included at least one annotated person.
...
45.25\% to 54.75\% ratio of frames with persons to frames without persons.

As a result, the final data distribution was as follows: the training dataset contained 82384 frames, the validation dataset included 11379 frames and the test dataset consisted of 11382 frames. Notably, all three datasets maintained the original 45.25\% person presence, guaranteeing that the model's performance evaluation remained accurate to the real-world data distribution.
...
Model training was conducted on a Linux server equipped with an Intel(R) Xeon(R) Silver 4214Y CPU @ 2.20GHz with 48 cores, a Quadro RTX 8000 GPU and 188 GB of RAM. Model training concluded after 165 epochs, taking 63.79 hours. Early stopping was triggered following 10 consecutive epochs without improvement. The training process utilized YOLO11's built-in data augmentation, a batch size of 16, a cosine annealing learning rate scheduler (Loshchilov \& Hutter, 2017), and early stopping after 10 epochs without improvement, with a maximum of 200 epochs.

The loss function of the YOLOv11 model comprises three main components: Box Loss, Classification Loss, and Distribution Focal Loss (DFL) (Li et al., 2020; Terven et al., 2024). Box Loss estimates the difference between predicted bounding boxes and ground truth boxes to assess the model's localization accuracy. Classification Loss measures the model's ability to properly identify detected objects; however, in our study, this component is less relevant due to our focus on a single class---faces. DFL improves the model's ability to detect challenging objects by prioritizing difficult-to-detect instances.
