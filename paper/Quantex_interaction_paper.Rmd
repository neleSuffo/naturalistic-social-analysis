...
```
In this research, we employed Ultralytics' YOLO11, the "latest iteration in the Ultralytics YOLO series of real-time object detectors" [@jocherUltralyticsYOLO112024]. Released in October 2024, YOLO11 features faster processing, improved efficiency, and enhanced spatial attention mechanisms, allowing it to focus more effectively on critical regions of an image compared to previous YOLO versions [@khanamYOLOv11OverviewKey2024]. These advancements make YOLO11 well-suited for training on our egocentric dataset, which captures dynamic movements from a chest-mounted camera perspective. 
...
### Training and Convergence
Model training was performed on the same Linux server used for person detection, with the same hardware specifications. The training process  concluded after `r face_num_epochs` epochs, taking 63.79 hours. Early stopping was triggered following 10 consecutive epochs without improvement. The training process utilized YOLO11’s built-in data augmentation, a batch size of 16, a cosine annealing learning rate scheduler [@loshchilovSGDRStochasticGradient2017], and early stopping after 10 epochs without improvement, with a maximum of 200 epochs. 
