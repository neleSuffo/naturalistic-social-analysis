...
In this research, we employed Ultralytics' YOLO11, the "latest iteration in the Ultralytics YOLO series of real-time object detectors" [@jocherUltralyticsYOLO112024]. YOLO11, which was released in October 2024, has faster processing, increased efficiency, and improved spatial attention techniques to focus on critical parts of a picture compared to prior Yolo versions [@khanamYOLOv11OverviewKey2024]. These improvements make YOLO11 suited for training on our egocentric dataset to detect people, which includes dynamic motions captured from camera perspectives at chest level. 

### Dataset Splitting and Balancing
Data preprocessing initiated with annotation from `r person_total_num_videos` videos and a total number of `r person_total_num_frames` frames. Before splitting the data into training, validation and testing subsets, our interest was in the proportion of frames containing annotated persons versus those without. Our analysis showed that `r person_class_to_total_ratio`% of the frames included at least one annotated person.

 To maintain this inherent distribution across all subsets, we initially applied a stratified split, ensuring that each subset—training, validation, and testing—preserved the original 
`r person_class_to_total_ratio`% to `r 100-person_class_to_total_ratio`% ratio of frames with persons to frames without persons.

As a result, the final data distribution was as follows: the training dataset contained `r num_person_frames_train` frames, the validation dataset included `r num_person_frames_val` frames and the test dataset consisted of `r num_person_frames_test` frames. Notably, all three datasets maintained the original `r person_class_to_total_ratio`% person presence, guaranteeing that the model's performance evaluation remained accurate to the real-world data distribution.
...
The YOLO11 model achieved a precision of `r person_precision` and a recall of `r person_recall` on the testing set, resulting in an F1-score of `r person_f1_score`. These metrics, summarized in figure \@ref(tab:face-detection-metrics-detailed) indicate the model’s strong performance in accurately identifying faces while minimizing errors. The precision-recall curve, displayed in table \@ref(fig:person-metrics), further illustrates this performance, with the curve remaining close to the top-left corner. This positioning signifies that the model maintains high precision and recall across various thresholds, underscoring its effectiveness in detecting faces with confidence.
...
It's important to note that YOLO11 does not include a predefined "face" class in its standard configuration. To address this, we utilized a version of YOLO11 pretrained on face detection tasks [@coddYOLOv11nfacedetection2024]. When applied to our dataset, this pretrained model achieved an F1-Score of `r face_pretrained_f1_score`. While this performance was promising, we believed there was room for improvement. Therefore, we fine-tuned the model further on our dataset to enhance its accuracy and better adapt it to our specific data characteristics.

### Dataset Splitting and Balancing
We started data preprocessing with a dataset comprising a total of `r total_num_face_frames` frames from `r face_total_num_videos` annotated videos. Prior to splitting this dataset into training, validation, and testing datasets, we analyzed the proportion of frames containing annotated faces versus those without. Our analysis revealed that 19% of the frames included at least one annotated face. To maintain this inherent distribution across all subsets, we initially applied a stratified split, ensuring that each subset—training, validation, and testing—preserved the original 
...
### Training and Convergence
Model training was conducted on a Linux server equipped with an Intel(R) Xeon(R) Silver 4214Y CPU @ 2.20GHz with 48 cores, a Quadro RTX 8000 GPU and 188 GB of RAM. Model training concluded after `r face_num_epochs` epochs, taking 63.79 hours. Early stopping was triggered following 10 consecutive epochs without improvement. The training process utilized YOLO11’s built-in data augmentation, a batch size of 16, a cosine annealing learning rate scheduler [@loshchilovSGDRStochasticGradient2017], and early stopping after 10 epochs without improvement, with a maximum of 200 epochs. 
