...
## Annotation Strategy
The dataset annotations cover four key elements: persons, faces, gaze direction, and objects the child interacts with. For each detected person (or reflection of a person, such as in a mirror) and face, additional attributes, such as age and gender [MB: specify exactly which categories these are], are collected. Gaze information indicates whether a detected person’s gaze is directed toward the child or not. Faces are annotated even when occluded or blurry to ensure comprehensive coverage of interactions. Partially visible faces are also annotated if key facial features, such as the nose, eyes, or mouth, remain identifiable. Objects are annotated only when the child is actively interacting with them [MB: that is, holding it in their hands?]. These objects are categorized into six distinct groups: book, screen, animal, food, toy, and kitchenware, with an additional category for other objects. The annotation strategy is summarized in Figure \@ref(fig:camera-cvat-activity-classes).

[MB: say somewhere a) how many videos/frames were annotated - not all of them were. Also say why they were annotated. and also that not all frames but only ever nth frame was annotated]
...
## Data Preprocessing
[MB:To prepare the video data for analysis, one frame per second was annotated, corresponding to every 30th frame in the video  should be included above - see comment. This frame sampling was chosen to balance the need for a representative sample of the video while keeping the analysis manageable]. [MB:weave in elsewhere so we don't need this section: Similarly, every 30th raw frame was extracted from the annotated video files for further processing. No preprocessing was applied to the audio data, which was used in its raw form for analysis.]
...
[MB: One paragraph outlining the structure of the supplement would be nice, see e.g. overview here: https://github.com/ccp-eva/laac/blob/master/writing/supplement/laac_supplement.Rmd.. Supplement will be a separate file.]


[MB: repeat the participant information here again because it is a separate file. Copy paste from above and add more details if there are more to add]
...
Model performance was evaluated using the F-measure [MB: if this is the same measure as above, introduce it above and then just use the label here], which balances precision and recall for each voice type. This metric is particularly important as it reflects both the accuracy of identifying each voice type (precision) and the model’s ability to detect all occurrences of that voice type (recall). Importantly, the Voice Type Classifier outperforms the LENA system [@fordLENALanguageEnvironment2008], a widely used but closed-source alternative, by a large margin [MB: give the number]. 
