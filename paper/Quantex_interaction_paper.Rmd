...
While our dataset originally included seven object categories, we merged the "animal" category with the "other object" category due to its low occurrence (only 19 instances), whereas all other categories had counts exceeding 1,000. As a result, our final model was trained on six object categories.
...
my.data <- "Class                   | Training  | Validation  | Testing | Total
            ...
            Food                    | 892       | 111         | 112     | 1115
            Other Object            | 14699     | 1838        | 1837    | 18374
...
Conversely, the "food" class shows significantly lower performance (AP = 0.619), compounded by its low prevalence in the dataset (0.98% of the total occurrences). The confusion matrix supports this observation, indicating that 36% of "food" items are frequently misclassified as "background". This misclassification is likely due to the limited number of training samples for this class (1115 total occurrences). Performance for "other\_object" (AP 0.820) also lags behind, along with "infant/child" (AP 0.857) and "infant/child face" (AP 0.854). "Kitchenware" class has AP score of 0.774. Analysis suggests a higher proportion of false negatives in the “food," "other object", "infant/child", and "infant/child face" classes than other classes.

The confusion matrix also indicates some confusion between "infant/child" and "background" classes, with 17% of images from the "infant/child" class identified as "background". In general, the object classes (food, other\_object, kitchenware) appear to be more challenging for the model. This may be due to the annotation strategy, which prioritized labeling only the objects with which the child directly interacts. Many other objects in the scene are not annotated and might be visually similar to the target objects, leading the model to become confused during training.

To address these issues, future work will focus on targeted data augmentation, with an emphasis on under-represented categories such as food, and address the specific problem of annotation, explore advanced training techniques that allow to penalize errors on the under-represented class more heavily, and implement a more comprehensive annotation protocol to account for all objects that might influence the model's classification.
