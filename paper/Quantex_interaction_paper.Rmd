...
```{r face-loss-curves, fig.align='center', fig.cap="Training and Validation Loss Curves for the YOLO11 face detection model.", out.width="300px"}
knitr::include_graphics("images/yolo_face_loss_curves.png")
```

- Yolo11 uses a loss function consisting of three components: box loss, class loss, and distribution focal loss (DFL) as displayed in \@ref(fig:face-loss-curves) [@coddYOLOv11nfacedetection2024]. 
...
```{r face-statistics, echo=FALSE, message=FALSE, warning=FALSE}
face_recall <- 0.86
num_faces_detected <- 2107
num_faces_correct <- 2062
num_faces_missed <- 402
num_wrong_faces <- 45
per_wrong_faces <- num_wrong_faces / num_faces_detected * 100
num_faces <- num_faces_detected + num_faces_missed
```
@jocherUltralyticsYolo11Performance2024


```{r face-metrics, fig.align='center', fig.cap="Evaluation metrics for the YOLO11 face detection model trained on the Quantex dataset."}
# Load images
img1 <- rasterGrob(as.raster(png::readPNG("images/yolo_face_confusion_matrix.png")), interpolate = TRUE)
img2 <- rasterGrob(as.raster(png::readPNG("images/yolo_face_pr_curve.png")), interpolate = TRUE)

# Create a blank plot and add images
p1 <- ggplot() + annotation_custom(img1, xmin=-1, xmax=1, ymin=-1, ymax=1) + theme_void()
p2 <- ggplot() + annotation_custom(img2, xmin=-1, xmax=1, ymin=-1, ymax=1) + theme_void()

# Arrange both plots side by side
grid.arrange(p1, p2, ncol=2)
```
- when looking at confusion matrix: you can see that the majority of images are correctly identified as face. `r face_recall`% of all faces are correctly identified by the model, resulting in a total of `r num_faces_detected` faces detected, while r num_faces_missed` weren't detected. 
...
- \@ref(fig:face-metrics) provides examples of correctly classified face images (true positives) and incorrectly classified face images (false negatives).
...
- a small amount of images are classified as faces while no face is present in the image (false positive). from the confusion matrix you can derive that the model wrongly classified only `r num_wrong_faces` images as face while there was no face present in the image. This results in a false positive rate of `r per_wrong_faces`%. Those cases can be caused by objects or toys that are similar to faces, an example of such a case is shown in \@ref(fig:face-detection-examples).

```{r face-detection-examples, fig.align='center', fig.cap="Examples of correctly classified face images (true positives) and incorrectly classified face images (false negatives) by the YOLO11 face detection model."}
img1 <- rasterGrob(as.raster(readPNG("images/quantex_at_home_id255944_2022_03_07_01_002760.jpg")), width = unit(1, "npc"), height = unit(1, "npc"), interpolate = TRUE)
img2 <- rasterGrob(as.raster(readPNG("images/quantex_at_home_id255944_2022_03_07_01_002760.jpg")), width = unit(1, "npc"), height = unit(1, "npc"), interpolate = TRUE)
img3 <- rasterGrob(as.raster(readPNG("images/quantex_at_home_id255944_2022_03_07_01_002760.jpg")), width = unit(1, "npc"), height = unit(1, "npc"), interpolate = TRUE)
img4 <- rasterGrob(as.raster(readPNG("images/quantex_at_home_id255944_2022_03_07_01_002760.jpg")), width = unit(1, "npc"), height = unit(1, "npc"), interpolate = TRUE)
img5 <- rasterGrob(as.raster(readPNG("images/quantex_at_home_id255944_2022_03_07_01_002760.jpg")), width = unit(1, "npc"), height = unit(1, "npc"), interpolate = TRUE)
img6 <- rasterGrob(as.raster(readPNG("images/quantex_at_home_id255944_2022_03_07_01_002760.jpg")), width = unit(1, "npc"), height = unit(1, "npc"), interpolate = TRUE)

# Create blank plots to hold images
...
p6 <- ggplot() + annotation_custom(img6) + theme_void()
...
### Model Training Time and Hardware Utilization
- The model was trained on a Linux server equipped with 48 cores and 187 GB of RAM.
- model was trained for a total of 86 epochs, after early stopping was applied after 10 epochs without improvement.
